...
function summarize(goodreads) {
  ...
  return {
    ...
    avgRating: readBooks.length ? (readBooks.reduce((s,b)=>s+(Number(b.myRating)||0),0)/readBooks.length).toFixed(2) : '0.00',
...
function excluded(candidate, idx) {
...
function matchScore(candidate, idx, profile, timesShown) {
  ...
  if (profile.medianPages && candidate.pages) {
    ...
    if (delta <= 50) score += 6; else if (delta <= 100) score += 3; else if (delta >= 220) score -= 4;
  }
  ...
  if (timesShown === 1) score -= 10; else if (timesShown === 2) score -= 25; else if (timesShown >= 3) score -= 50;
...
function confidenceScore(candidate, idx) {
  ...
  const overlap = (candidate.similarToAuthors || []).reduce((sum, a) => sum + (idx.fiveStarAuthors.get(a) || 0), 0);
...
function reason(candidate, idx) {
  ...
  return parts.length ? `Recommended because it ${parts.join('; ')}.` : 'Recommended because it aligns with your overall profile.';
}
...
export function rankRecommendations(goodreads, feedback, candidatePool, history) {
  ...
  const viable = (candidatePool || []).filter(c => !excluded(c, idx));
  ...
  const selected = [...fiction, ...nonfiction].sort((a,b) => (b.matchScore-a.matchScore)||(b.confidenceScore-a.confidenceScore)).map((x,i)=>({ ...x, rank:i+1 }));
  return { selected, profile, eligibleCount: viable.length, fictionPool: scored.filter(x=>x.type==='fiction').length, nonfictionPool: scored.filter(x=>x.type==='nonfiction').length };
}
