...
const state = { goodreads:null, feedback:null, history:null, candidates:null, ranking:null, pending:null, imported:null };
...
function tile(label, value){ return `<div class="tile"><span class="tile-label">${label}</span><div class="tile-value">${value}</div></div>`; }
...
function renderAnalytics(){
  ...
  analyticsTiles.innerHTML = [
    ...
    tile('Recent read year', p.mostRecentReadYear || 'n/a')
  ].join('');
  const topAuthors = p.favoriteAuthors.slice(0,3).map(([name])=>name).join(', ');
  const insights = [
    `Strongest repeat-author signal: ${topAuthors || 'not enough data yet'}.`,
    `Eligible pool after exclusions: ${state.ranking.eligibleCount} candidates (${state.ranking.fictionPool} fiction / ${state.ranking.nonfictionPool} nonfiction).`,
    `Write mode is ${isWriteConfigured() ? 'enabled' : 'disabled'}; ${isWriteConfigured() ? 'dismissals will persist back to GitHub.' : 'dismissals are local until you configure storage.js.'}`
  ];
...
function renderRecommendations(){
  if (!state.ranking.selected.length){
    recommendationsGrid.innerHTML = `<div class="card"><h3>No eligible recommendations</h3><p class="note">Import Goodreads data or expand the candidate pool.</p></div>`;
    return;
  }
  recommendationsGrid.innerHTML = state.ranking.selected.map(book => `
    <article class="card">
      <div class="card-top">
        ...
        <span class="type-badge">${book.type}</span>
      </div>
      <div>
        <h3>${book.title}</h3>
      ...
      <div><span class="small-label">Why this fits</span><p>${book.reason}</p></div>
      <div><span class="small-label">Similar to</span><ul class="similar-list">${[...(book.similarToTitles||[]), ...(book.similarToAuthors||[])].slice(0,3).map(x=>`<li>${x}</li>`).join('')}</ul></div>
...
async function dismiss(reasonCode){
  ...
  if (!row) {
    row = { bookKey: book.bookKey, firstShownDate: new Date().toISOString(), lastShownDate: new Date().toISOString(), timesShown:1, timesDismissed:1, timesSaved:0, timesClicked:0 };
  ...
  if (isWriteConfigured()) {
    try {
      ...
      await safeUpdateRepoJson('data/recommendationHistory.json', content => {
        ...
        const row = content.history.find(x => x.bookKey === book.bookKey);
        if (row) {
          row.timesDismissed = (row.timesDismissed || 0) + 1;
          row.lastShownDate = new Date().toISOString();
        } else {
          content.history.push({ bookKey: book.bookKey, firstShownDate:new Date().toISOString(), lastShownDate:new Date().toISOString(), timesShown:1, timesDismissed:1, timesSaved:0, timesClicked:0 });
        }
        content.meta = content.meta || {};
        content.meta.lastUpdated = new Date().toISOString();
        return content;
      }, `History ${book.title}`);
      setStatus(`Dismissed ${book.title} and saved to GitHub.`);
    } catch (e) {
...
processCsvButton.addEventListener('click', async () => {
  ...
  if (!file) { importStatus.textContent = 'Choose a Goodreads CSV first.'; return; }
...
saveRepoButton.addEventListener('click', async () => {
  ...
  try {
    ...
    setStatus('Saved Goodreads JSON back to the repo.');
  } catch (e) {
    setStatus(`Save failed: ${e.message}`);
  }
});

toggleImportButton.addEventListener('click', () => importSection.classList.toggle('hidden'));
...
async function initialize(force = false){
  ...
  recompute();
  setStatus(`Loaded ${state.goodreads.meta.readCount || 0} read books, ${state.feedback.interactions.length} feedback records, and ${state.candidates.length} starter candidates.`);
}

initialize().catch(error => setStatus(`Failed to load app: ${error.message}`));
