...
one run is one Claude call (~15-25 web searches, a few thousand output
tokens) plus up to 10 cheap TMDB search calls. That's a few cents to
perhaps $0.50/run, not the "tens of dollars per pass" scale that got the
...
Return exactly 10 shows or limited series currently airing/streaming (a mix of platforms and genres is good — don't let one platform or genre dominate unless that's genuinely what the data shows). For each, assign exactly one tag: HOT (breakout, dominating conversation), RISING (real upward momentum), BUZZY (talked about, may not be #1 in raw viewership), ESTABLISHED (a proven, still-strong performer), or UNDER-THE-RADAR (genuinely good, deserves more attention than it's getting).
...
{{
  "shows": [
    {{
      ...
      "whyHere": "1-2 sentences, the real data/momentum reason it's ranked here this week",
...
Rules:
- All 10 "rank" values 1-10, no gaps or repeats, ordered by your real composite score (highest first).
...
def call_claude():
    body = json.dumps({
        ...
        'messages': [{'role': 'user', 'content': PROMPT.format(
            today=datetime.now(timezone.utc).strftime('%B %d, %Y'), methodology=METHODOLOGY)}],
    }).encode()
...
def parse_shows():
    ...
    if not isinstance(shows, list) or len(shows) != 10:
        raise ValueError(f'expected exactly 10 shows, got {len(shows) if isinstance(shows, list) else "non-list"}')
    ranks = sorted(s.get('rank') for s in shows)
    if ranks != list(range(1, 11)):
        raise ValueError(f'ranks are not a clean 1-10 sequence: {ranks}')
    return shows
...
def main():
    ...
    print(f'Wrote {OUT_FILE} — {sum(1 for s in shows if s["titleKey"])}/10 shows resolved to a real TMDB id.')
