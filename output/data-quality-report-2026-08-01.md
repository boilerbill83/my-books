...
Generated by `scripts/data_quality_report.js`. Total books analyzed: **1126** (to-read: 297, read: 649, currently-reading: 2, candidate-pool: 178).

## Data Quality Score: 69/100

- Critical Field Health (8%): 99.9/100
- Field Completeness (33%): 68.0/100
- Foundational Signal Integrity (8%): 100.0/100
- Recommendation Safety (11%): 95.6/100
- Data Hygiene (40%): 50.1/100
...
- 🔁 **2 self-referential entries** (a book citing itself or its own author).
...
- "Medium Raw: A Bloody Valentine to the World of Food and the People Who Cook" by Anthony Bourdain (score 20) — self-referential similarToTitles/similarToAuthors entry
- "The New Jim Crow: Mass Incarceration in the Age of Colorblindness" by Michelle Alexander (score 20) — self-referential similarToTitles/similarToAuthors entry
...
- Integrity count changes: selfReferential +2, brokenNearMiss +25, brokenOrphaned -74
...
- "Medium Raw: A Bloody Valentine to the World of Food and the People Who Cook" by Anthony Bourdain (candidate_pool/candidate-pool) — score 20: self-referential similarToTitles/similarToAuthors entry
- "The New Jim Crow: Mass Incarceration in the Age of Colorblindness" by Michelle Alexander (candidate_pool/candidate-pool) — score 20: self-referential similarToTitles/similarToAuthors entry
...
**Percent Populated** is raw — % of all 1126 books with a non-empty value.
...
|---|---|---|---|---|
| isbn ⚠️ | 64.0% (target: 129 eligible, not all 1,126) | 64.3% (Fair) | % of books published before 2007 (when ISBN-13 became the sole standard) with an ISBN-10. Post-2007 books are excluded from the denominator since most were never issued one. | 46 pre-2007 book(s) missing ISBN-10 — isbn13 (the modern identifier) is the field that actually matters; only chase this if isbn13 is also missing for the same book. |
| dateRead ⚠️ | 41.5% (target: 649 eligible, not all 1,126) | 72.0% (Fair) | % of READ-shelf books with a read date — to-read/candidate books have none by definition. | 182 read book(s) missing a read date — check for legacy import gaps. |
| subjects ⚠️ | 74.0% (target: 1,109 eligible, not all 1,126) | 75.1% (Fair) | % of books ALREADY ATTEMPTED by enrich_metadata.py that got subject tags from Open Library — separates backlog from real Open Library coverage gaps. | 276 attempted book(s) got no subjects — real Open Library coverage gap (varies 60-95% by book type per Session 13e analysis); not worth chasing further without a different data source. |
| dismissReason | 1.8% (target: 25 eligible, not all 1,126) | 80.0% (Good) | % of ACTUALLY DISMISSED books (dismissed==true) that have a reasonLabel — scoring against all 1117 books would be misleading since only ~2% have any feedback at all by design. | 5 dismissed book(s) have a reasonCode but no reasonLabel in feedbackData.json — backfill the label or have the extract fall back to reasonCode. |
| dateAdded | 82.6% | 82.6% (Good) | % of all books with a dateAdded. | Low-volume gap; not used by scoring, cosmetic only. |
| publisher | 85.8% | 85.8% (Good) | % of all books with a publisher. | Backfill gaps via Google Books lookup if needed for the extract; not used by the scoring engine. |
| amazonRating | 38.9% (target: 475 eligible, not all 1,126) | 89.7% (Good) | % of to-read/candidate-pool books scraped by scrape_ratings.py that returned a real rating (source=="amazon") — already-read books are never scraped since the engine never recommends them, so they are excluded from the denominator. | 49 book(s) in the eligible pool have no Amazon rating. As of Session 13d these are retried automatically after a `RETRY_COOLDOWN_DAYS` cooldown instead of being cached as permanent misses — re-check after a few scheduled runs before investigating further. |
| amazonRatingsCount | 38.9% (target: 475 eligible, not all 1,126) | 89.7% (Good) | Same eligibility and mechanism as amazonRating (written together by scrape_ratings.py). | Same as amazonRating — tracks it exactly. |
| categories | 94.9% (target: 1,109 eligible, not all 1,126) | 96.4% (Excellent) | % of books ALREADY ATTEMPTED by enrich_metadata.py (has a metadataFetchedAt) that got a category back from Google Books — separates "not yet attempted" (backlog) from "attempted, no data." | 40 attempted book(s) got no category — Google Books coverage gap, not fixable by retrying with current logic. |
| coverUrl | 51.2% (target: 477 eligible, not all 1,126) | 97.1% (Excellent) | % of books on shelves enrich_covers.py actually targets (to-read/candidates/currently-reading) with a cover — read-shelf books are excluded from the denominator since that job skips them by design. | 14 book(s) in the eligible pool still missing a cover — re-run enrich_covers.py. Read-shelf covers are a separate, larger gap outside this job's current scope (raised in Session 13e, not yet actioned). |
...
| myRating | 84.1% (target: 649 eligible, not all 1,126) | 100.0% (Excellent) | % of READ-shelf books with a rating — to-read/candidate books are unrated by definition, so they are excluded from the denominator. Feeds the 5-star-author bonus and authorRatingWeight directly. | None needed; this is a read-only field. |
...
### Self-referential entries (2)
- "Medium Raw: A Bloody Valentine to the World of Food and the People Who Cook" by Anthony Bourdain lists itself in similarToTitles
- "The New Jim Crow: Mass Incarceration in the Age of Colorblindness" by Michelle Alexander lists itself in similarToTitles
...
**Near-miss (77, 0 touching a 5★ read)** — a close match exists in the dataset (substring/prefix overlap or a small edit distance); almost always a fixable typo, truncation, or formatting difference. Sorted so refs touching a 5★ read (corrupting a live scoring signal) come first.
...
- "The Origins of the Cornbread Mafia: A Memoir of Sorts" cites "Dreamland"
...
- "A Knock at Midnight: A Story of Hope, Justice, and Freedom" cites "The New Jim Crow"
- "A People's History of the United States" cites "The New Jim Crow"
- "The Last Human" cites "A Long Way to a Small Angry Planet"
- "Fentanyl, Inc.: How Rogue Chemists Are Creating the Deadliest Wave of the Opioid Epidemic" cites "Dreamland"
...
- "Lucky Peach: Issue 1" cites "Medium Raw"
- "Rise of the Warrior Cop: The Militarization of America's Police Forces" cites "The New Jim Crow"
...
- "The Body: A Guide for Occupants" cites "The Emperor of All Maladies"
- "The Body: A Guide for Occupants" cites "Being Mortal"
...
**Orphaned (382)** — no close match anywhere in the dataset; likely a reference to a book that was never added at all. Lower priority than near-miss.
- "Shit, Actually: The Definitive, 100% Objective Guide to Modern Cinema" cites "The Wreckage of My Presence"
- "The Ride of a Lifetime: Lessons Learned from 15 Years as CEO of the Walt Disney Company" cites "What It Takes: Lessons in the Pursuit of Excellence"
- "Quiet Power: The Secret Strengths of Introverts" cites "Bittersweet: How Sorrow and Longing Make Us Whole"
- "Quiet: The Power of Introverts in a World That Can't Stop Talking" cites "Bittersweet: How Sorrow and Longing Make Us Whole"
- "To Start a War: How the Bush Administration Took America into Iraq" cites "Fiasco"
- "To Start a War: How the Bush Administration Took America into Iraq" cites "Imperial Life in the Emerald City"
- "To Start a War: How the Bush Administration Took America into Iraq" cites "The Operators"
- "To Start a War: How the Bush Administration Took America into Iraq" cites "Bush at War"
- "Once Upon a Time in Hollywood: A Novel" cites "L.A. Confidential"
- "Once Upon a Time in Hollywood: A Novel" cites "White Jazz"
- "Once Upon a Time in Hollywood: A Novel" cites "Less Than Zero"
- "Once Upon a Time in Hollywood: A Novel" cites "Inherent Vice"
- "The Universal Baseball Association, Inc., J. Henry Waugh, Prop." cites "The Great American Novel"
- "The Universal Baseball Association, Inc., J. Henry Waugh, Prop." cites "End Zone"
- "The Universal Baseball Association, Inc., J. Henry Waugh, Prop." cites "The Franchise"
- "Zone One" cites "A Head Full of Ghosts"
- "Zone One" cites "The Passage"
- "The Origins of the Cornbread Mafia: A Memoir of Sorts" cites "The Bluegrass Conspiracy"
- "Liftoff: Elon Musk and the Desperate Early Days That Launched SpaceX" cites "Rocket Billionaires"
- "A Knock at Midnight: A Story of Hope, Justice, and Freedom" cites "Between the World and Me"
- …and 362 more
...
1. **Orphaned similarToTitles references** (−14.1 pts)
   382 reference(s) with no close match anywhere in the dataset — mostly legitimate citations to books never added, but at this volume (8.2% of all citations) it's a real completeness gap on the similarToTitles signal, not nothing.
   **Fix:** Add the missing books to the dataset where practical, or confirm the reference is intentionally external.

2. **Field completeness gaps (all fields)** (−10.6 pts)
   Average Quality Score across all 31 tracked fields is 94.4% — isbn, dateRead, subjects, tones, and publisher are the largest gaps, none of them scoring-critical but all real incompleteness for a genuinely excellent dataset.
   **Fix:** See the Field Population table, sorted by Quality Score, for every field below 90%.

3. **Broken similarToTitles references (candidate-to-candidate)** (−5.6 pts)
   77 near-miss reference(s) between books that aren't 5★ reads — lower live-scoring impact, but still fixable typos or subtitle truncations.
...
5. **Self-referential entries** (−0.3 pts)
   2 book(s) cite themselves in similarToTitles or list their own author in similarToAuthors — a data-entry mistake, not a useful signal.
...
**Current** — projected score: 69/100


**Phase 1 — Quick data-entry fixes** — projected score: 76/100 (+7 from current)
...
**Phase 2 — Add the top 30 missing books** — projected score: 80/100 (+11 from current)
...
**Phase 3 — Add the next-tier missing books (top 100)** — projected score: 83/100 (+14 from current)
...
**Phase 4 — Automation catch-up + targeted backfills** — projected score: 88/100 (+19 from current)
...
**Theoretical ceiling (literal 100)** — projected score: 100/100 (+31 from current)
