...
Generated by `scripts/data_quality_report.js`. Total books analyzed: **1125** (to-read: 297, read: 649, currently-reading: 2, candidate-pool: 177).

## Data Quality Score: 67/100

- Critical Field Health (8%): 99.9/100
- Field Completeness (33%): 68.2/100
- Foundational Signal Integrity (8%): 100.0/100
- Recommendation Safety (11%): 95.6/100
- Data Hygiene (40%): 45.9/100
...
- Integrity count changes: brokenNearMiss +25, brokenOrphaned -59
...
**Percent Populated** is raw — % of all 1125 books with a non-empty value.
...
|---|---|---|---|---|
| isbn ⚠️ | 64.1% (target: 128 eligible, not all 1,125) | 64.8% (Fair) | % of books published before 2007 (when ISBN-13 became the sole standard) with an ISBN-10. Post-2007 books are excluded from the denominator since most were never issued one. | 45 pre-2007 book(s) missing ISBN-10 — isbn13 (the modern identifier) is the field that actually matters; only chase this if isbn13 is also missing for the same book. |
| dateRead ⚠️ | 41.5% (target: 649 eligible, not all 1,125) | 72.0% (Fair) | % of READ-shelf books with a read date — to-read/candidate books have none by definition. | 182 read book(s) missing a read date — check for legacy import gaps. |
| subjects ⚠️ | 74.0% (target: 1,109 eligible, not all 1,125) | 75.1% (Fair) | % of books ALREADY ATTEMPTED by enrich_metadata.py that got subject tags from Open Library — separates backlog from real Open Library coverage gaps. | 276 attempted book(s) got no subjects — real Open Library coverage gap (varies 60-95% by book type per Session 13e analysis); not worth chasing further without a different data source. |
| dismissReason | 1.8% (target: 25 eligible, not all 1,125) | 80.0% (Good) | % of ACTUALLY DISMISSED books (dismissed==true) that have a reasonLabel — scoring against all 1117 books would be misleading since only ~2% have any feedback at all by design. | 5 dismissed book(s) have a reasonCode but no reasonLabel in feedbackData.json — backfill the label or have the extract fall back to reasonCode. |
...
| amazonRating | 38.9% (target: 474 eligible, not all 1,125) | 89.9% (Good) | % of to-read/candidate-pool books scraped by scrape_ratings.py that returned a real rating (source=="amazon") — already-read books are never scraped since the engine never recommends them, so they are excluded from the denominator. | 48 book(s) in the eligible pool have no Amazon rating. As of Session 13d these are retried automatically after a `RETRY_COOLDOWN_DAYS` cooldown instead of being cached as permanent misses — re-check after a few scheduled runs before investigating further. |
| amazonRatingsCount | 38.9% (target: 474 eligible, not all 1,125) | 89.9% (Good) | Same eligibility and mechanism as amazonRating (written together by scrape_ratings.py). | Same as amazonRating — tracks it exactly. |
| categories | 95.0% (target: 1,109 eligible, not all 1,125) | 96.4% (Excellent) | % of books ALREADY ATTEMPTED by enrich_metadata.py (has a metadataFetchedAt) that got a category back from Google Books — separates "not yet attempted" (backlog) from "attempted, no data." | 40 attempted book(s) got no category — Google Books coverage gap, not fixable by retrying with current logic. |
| coverUrl | 51.3% (target: 476 eligible, not all 1,125) | 97.3% (Excellent) | % of books on shelves enrich_covers.py actually targets (to-read/candidates/currently-reading) with a cover — read-shelf books are excluded from the denominator since that job skips them by design. | 13 book(s) in the eligible pool still missing a cover — re-run enrich_covers.py. Read-shelf covers are a separate, larger gap outside this job's current scope (raised in Session 13e, not yet actioned). |
...
| metadataFetchedAt | 98.6% | 98.6% (Excellent) | % of all books already attempted by enrich_metadata.py — this IS the backlog-cleared indicator for categories/subjects/description above. | 16 book(s) not yet attempted — will clear at 150/day, prioritizing to-read then read then candidates. |
...
| myRating | 84.2% (target: 649 eligible, not all 1,125) | 100.0% (Excellent) | % of READ-shelf books with a rating — to-read/candidate books are unrated by definition, so they are excluded from the denominator. Feeds the 5-star-author bonus and authorRatingWeight directly. | None needed; this is a read-only field. |
...
**Orphaned (397)** — no close match anywhere in the dataset; likely a reference to a book that was never added at all. Lower priority than near-miss.
...
1. **Orphaned similarToTitles references** (−15.6 pts)
   397 reference(s) with no close match anywhere in the dataset — mostly legitimate citations to books never added, but at this volume (9.1% of all citations) it's a real completeness gap on the similarToTitles signal, not nothing.
   **Fix:** Add the missing books to the dataset where practical, or confirm the reference is intentionally external.

2. **Field completeness gaps (all fields)** (−10.5 pts)
