...
- Integrity count changes: brokenNearMiss +25, brokenOrphaned -75, similarOutOfRange -1
...
**Orphaned (381)** — no close match anywhere in the dataset; likely a reference to a book that was never added at all. Lower priority than near-miss.
...
- similarToTitles outside 3–5 range: **3** ("Careless People: A Cautionary Tale of Power, Greed, and Lost Idealism", "Anatomy of an Alibi", "The Inmate")
...
1. **Orphaned similarToTitles references** (−12.7 pts)
   381 reference(s) with no close match anywhere in the dataset — mostly legitimate citations to books never added, but at this volume (7.4% of all citations) it's a real completeness gap on the similarToTitles signal, not nothing.
...
3. **Broken similarToTitles references (candidate-to-candidate)** (−5.1 pts)
...
**Current** — projected score: 71/100


**Phase 1 — Quick data-entry fixes** — projected score: 78/100 (+7 from current)
...
**Phase 2 — Add the top 30 missing books** — projected score: 81/100 (+10 from current)
...
**Phase 3 — Add the next-tier missing books (top 100)** — projected score: 84/100 (+13 from current)
...
**Phase 4 — Automation catch-up + targeted backfills** — projected score: 89/100 (+18 from current)
...
**Theoretical ceiling (literal 100)** — projected score: 100/100 (+29 from current)
