...
Generated by `scripts/data_quality_report.js`. Total books analyzed: **1107** (to-read: 297, read: 649, currently-reading: 2, candidate-pool: 159).

## Data Quality Score: 90/100

- Critical Field Health (8%): 100.0/100
- Field Completeness (33%): 68.8/100
...
A separate measure from the Data Quality Score above — this one is not about whether the data is complete/correct, but whether the engine's actual predictions are good, via `scripts/eval.js`'s leave-one-out evaluation over 503 completed rated reads.
...
- Integrity count changes: dupBookKeys -2, dupTitleAuthor -3, brokenNearMiss -52, brokenOrphaned -456, similarOutOfRange -4
...
**Percent Populated** is raw — % of all 1107 books with a non-empty value.
...
|---|---|---|---|---|
| isbn ⚠️ | 65.1% (target: 126 eligible, not all 1,107) | 65.9% (Fair) | % of books published before 2007 (when ISBN-13 became the sole standard) with an ISBN-10. Post-2007 books are excluded from the denominator since most were never issued one. | 43 pre-2007 book(s) missing ISBN-10 — isbn13 (the modern identifier) is the field that actually matters; only chase this if isbn13 is also missing for the same book. |
| dateRead ⚠️ | 42.2% (target: 649 eligible, not all 1,107) | 72.0% (Fair) | % of READ-shelf books with a read date — to-read/candidate books have none by definition. | 182 read book(s) missing a read date — check for legacy import gaps. |
| subjects ⚠️ | 73.8% (target: 1,090 eligible, not all 1,107) | 75.0% (Fair) | % of books ALREADY ATTEMPTED by enrich_metadata.py that got subject tags from Open Library — separates backlog from real Open Library coverage gaps. | 273 attempted book(s) got no subjects — real Open Library coverage gap (varies 60-95% by book type per Session 13e analysis); not worth chasing further without a different data source. |
| dismissReason | 1.8% (target: 25 eligible, not all 1,107) | 80.0% (Good) | % of ACTUALLY DISMISSED books (dismissed==true) that have a reasonLabel — scoring against all 1117 books would be misleading since only ~2% have any feedback at all by design. | 5 dismissed book(s) have a reasonCode but no reasonLabel in feedbackData.json — backfill the label or have the extract fall back to reasonCode. |
| dateAdded | 84.0% | 84.0% (Good) | % of all books with a dateAdded. | Low-volume gap; not used by scoring, cosmetic only. |
| publisher | 87.1% | 87.1% (Good) | % of all books with a publisher. | Backfill gaps via Google Books lookup if needed for the extract; not used by the scoring engine. |
| amazonRating | 38.0% (target: 456 eligible, not all 1,107) | 89.7% (Good) | % of to-read/candidate-pool books scraped by scrape_ratings.py that returned a real rating (source=="amazon") — already-read books are never scraped since the engine never recommends them, so they are excluded from the denominator. | 47 book(s) in the eligible pool have no Amazon rating. As of Session 13d these are retried automatically after a `RETRY_COOLDOWN_DAYS` cooldown instead of being cached as permanent misses — re-check after a few scheduled runs before investigating further. |
| amazonRatingsCount | 38.0% (target: 456 eligible, not all 1,107) | 89.7% (Good) | Same eligibility and mechanism as amazonRating (written together by scrape_ratings.py). | Same as amazonRating — tracks it exactly. |
| categories | 94.9% (target: 1,090 eligible, not all 1,107) | 96.3% (Excellent) | % of books ALREADY ATTEMPTED by enrich_metadata.py (has a metadataFetchedAt) that got a category back from Google Books — separates "not yet attempted" (backlog) from "attempted, no data." | 40 attempted book(s) got no category — Google Books coverage gap, not fixable by retrying with current logic. |
| coverUrl | 50.4% (target: 458 eligible, not all 1,107) | 96.9% (Excellent) | % of books on shelves enrich_covers.py actually targets (to-read/candidates/currently-reading) with a cover — read-shelf books are excluded from the denominator since that job skips them by design. | 14 book(s) in the eligible pool still missing a cover — re-run enrich_covers.py. Read-shelf covers are a separate, larger gap outside this job's current scope (raised in Session 13e, not yet actioned). |
| isbn13 | 98.1% | 98.1% (Excellent) | % of all books with an ISBN-13 — enrich_isbn.py only targets to-read/candidate shelves, so read-shelf gaps rely on the original Goodreads import. | 21 book(s) missing isbn13. If concentrated on the read shelf, extend enrich_isbn.py to also backfill read books (currently out of scope by design). |
...
| myRating | 85.5% (target: 649 eligible, not all 1,107) | 100.0% (Excellent) | % of READ-shelf books with a rating — to-read/candidate books are unrated by definition, so they are excluded from the denominator. Feeds the 5-star-author bonus and authorRatingWeight directly. | None needed; this is a read-only field. |
...
1. **Field completeness gaps (all fields)** (−10.3 pts)
