...
- 🔁 **1 self-referential entry** (a book citing itself or its own author).
...
- Integrity count changes: selfReferential +1, brokenNearMiss +25, brokenOrphaned -74
...
1. **Orphaned similarToTitles references** (−14.4 pts)
   382 reference(s) with no close match anywhere in the dataset — mostly legitimate citations to books never added, but at this volume (8.4% of all citations) it's a real completeness gap on the similarToTitles signal, not nothing.
...
3. **Broken similarToTitles references (candidate-to-candidate)** (−5.7 pts)
...
5. **Self-referential entries** (−0.1 pts)
   1 book(s) cite themselves in similarToTitles or list their own author in similarToAuthors — a data-entry mistake, not a useful signal.
