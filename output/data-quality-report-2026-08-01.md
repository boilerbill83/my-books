...
- `dismissReason`: 📉 regressed 78.3% → 72.0% (-6.3 pts)
...
| dismissReason ⚠️ | 1.6% (target: 25 eligible, not all 1,112) | 72.0% (Fair) | % of ACTUALLY DISMISSED books (dismissed==true) that have a reasonLabel — scoring against all 1117 books would be misleading since only ~2% have any feedback at all by design. | 7 dismissed book(s) have a reasonCode but no reasonLabel in feedbackData.json — backfill the label or have the extract fall back to reasonCode. |
...
**Orphaned (456)** — no close match anywhere in the dataset; likely a reference to a book that was never added at all. Lower priority than near-miss.
...
- "Yes, Chef" cites "Blood, Bones, and Butter: The Inadvertent Education of a Reluctant Chef"
- "Smokin' with Myron Mixon: Recipes Made Simple, from the Winningest Man in Barbecue: A Cookbook" cites "Blood, Bones, and Butter: The Inadvertent Education of a Reluctant Chef"
- "Hungry: Eating, Road-Tripping, and Risking It All with the Greatest Chef in the World" cites "Blood, Bones, and Butter: The Inadvertent Education of a Reluctant Chef"
- "Three Women" cites "Ghost Lover"
- "Newjack: Guarding Sing Sing" cites "Cheap Land Colorado: Off-Gridders at America's Edge"
- "The Prophets of Smoked Meat: A Journey Through Texas Barbecue" cites "Blood, Bones, and Butter: The Inadvertent Education of a Reluctant Chef"
- …and 436 more
...
1. **Orphaned similarToTitles references** (−18.2 pts)
   456 reference(s) with no close match anywhere in the dataset — mostly legitimate citations to books never added, but at this volume (10.6% of all citations) it's a real completeness gap on the similarToTitles signal, not nothing.
   **Fix:** Add the missing books to the dataset where practical, or confirm the reference is intentionally external.

2. **Field completeness gaps (all fields)** (−10.2 pts)
   Average Quality Score across all 31 tracked fields is 94.6% — isbn, dateRead, subjects, tones, and publisher are the largest gaps, none of them scoring-critical but all real incompleteness for a genuinely excellent dataset.
