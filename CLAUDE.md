...
| `scripts/data_quality_report.js` | Weekly data quality report — per-field table (Field Name, Percent Populated, Quality Score, How Score is Determined, Suggestions for Improvement) via `FIELD_REGISTRY`, scoring each field against its *eligible* row subset rather than all books, plus integrity checks (duplicate bookKeys, broken similarToTitles refs, non-canonical themes, out-of-range field counts). Writes `output/data-quality-report-YYYY-MM-DD.md` |
